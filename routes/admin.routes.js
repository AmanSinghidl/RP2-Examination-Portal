    ...
    db.query(
        ...
        SELECT r.result_id,
               ...
               CASE
                   ...
                   WHEN q.total_questions > 0 AND r.total_marks IS NOT NULL
                       AND r.total_marks * 100 >= q.total_questions * COALESCE(ev.cutoff_percentage, 0)
                       THEN 'PASS'
                   WHEN q.total_questions > 0 AND r.total_marks IS NOT NULL THEN 'FAIL'
                   ELSE 'PENDING'
               END AS pass_fail
        FROM results r
